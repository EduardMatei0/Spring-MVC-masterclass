...
<body>
	...
	<div align="center">
		...
		<h3>Organization Country: <b>${orgreg.country}</b></h3> 
		...
		<h3>Type: 
			<c:forEach var="entry" items="${typeslist}">
		...
		<h3>Registered : 
			<c:forEach var="entry" items="${registeredList}">
				<c:if test="${orgreg.registeredPreviosly eq entry.key}">
					<b>${entry.value}</b>
				</c:if>
			</c:forEach>
		</h3>
		...
		<h3>Like our website:
			<c:choose>
				<c:when test="${orgreg.like eq 'yes' }"><b>Yes</b></c:when>
		...
		<h3>WorkForceSize : 
			<c:forEach var="entry" items="${employeeStengthList}">
